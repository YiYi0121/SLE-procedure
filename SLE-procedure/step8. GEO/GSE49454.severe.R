...
##### 读取分组数据 ####
table(group$group)
severe.group <- group %>% filter(group=="Healthy control" | group=="Severe") %>%
  mutate(group=case_when(group=="Healthy control" ~ "control",
                         group=="Severe" ~ "severe"))
severe.exp <- exp_SLE[,which(colnames(exp_SLE) %in% severe.group$GSM)]
severe.exp.output <- severe.exp %>% rownames_to_column("Gene") 
write.table(severe.exp.output, file="./severe.results/severe.exp.txt", sep="\t", quote=F, col.names=T,row.names =F)

##### 分组矩阵 ####
severe.rt<-as.matrix(severe.exp)
severe.class <- as.character(severe.group$group)
severe.design <- model.matrix(~0+factor(severe.class)) 
colnames(severe.design) <- c("control","severe")
table(severe.group$group)


#绘制归一化结果：经过归一化处理后，在每个样本中，数据的表达平齐了
col <- c(rep("skyblue",table(severe.group$group)[1]),rep("pink",table(severe.group$group)[2]))
pdf("./severe.results/Figure 6E.severe SLE and control data set preprocessing normalization effect.pdf", height = 6, width = 6) 
par(mar=c(6,5,4,3) + 0.2)
boxplot(severe.exp,outline=FALSE,notch=T,col=col,las=2,cex.lab=0.5)
dev.off()


#differential
severe.fit <- lmFit(severe.rt,severe.design)
severe.cont<-makeContrasts(severe-control,levels=severe.design)
severe.fit2 <- contrasts.fit(severe.fit, severe.cont)
severe.fit2 <- eBayes(severe.fit2)
severe.allDiff <- topTable(severe.fit2,adjust='fdr',number=200000)
##输出差异结果
logFoldChange=0.58
adjustP=0.05
severe.diffSig <- severe.allDiff[with(severe.allDiff, (abs(logFC)>logFoldChange & adj.P.Val < adjustP )), ]
severe.diffSig <- severe.diffSig[order(-severe.diffSig$logFC),] %>%
  mutate(regulate=case_when(logFC>0~"up",logFC<0~"down"))
table(severe.diffSig$regulate)
severe.diffSigOut <- rbind(id=colnames(severe.diffSig),severe.diffSig)
write.table(severe.diffSigOut, file="./severe.results/severe.diff.txt", sep="\t", quote=F, col.names=F)
#输出差异基因表达量
severe.diffGeneExp<- severe.rt[rownames(severe.diffSig),]
severe.diffGeneExpOut<- rbind(id=colnames(severe.diffGeneExp),severe.diffGeneExp)
write.table(severe.diffGeneExpOut,file="./severe.results/severe.diffGeneExp.txt",sep="\t",quote=F,col.names=F)
#绘制火山图
#定义显著性
library(ggplot2)
severe.Sig=ifelse((severe.allDiff$adj.P.Val<adjustP) & (abs(severe.allDiff$logFC)>logFoldChange), ifelse(severe.allDiff$logFC>logFoldChange,"Up","Down"), "Stable")
severe.hs=cbind(severe.allDiff, severe.Sig=severe.Sig)
severe.p <- ggplot(
  # 数据、映射、颜色
  severe.hs, aes(x = logFC, y = -log10(adj.P.Val), colour=severe.Sig)) +
...
#输出火山图
pdf(file="./severe.results/Figure 6F.severe SLE vs control.volcano.pdf", width=6, height=6)
print(severe.p)
dev.off()

#轻型和重型SLE差异基因交集
mild.diffSig <-  read.table("C:/Users/Administrator/Desktop/SLE/procedure/step8. GEO/mild.results/mild.diff.txt",header = T, sep="\t",row.names = 1) 

jj <- list()
jj[["Mild SLE DEGs"]]=unique(rownames(mild.diffSig))
jj[["Severe SLE DEGs"]]=unique(rownames(severe.diffSig))

#绘制venn图
library(venn)
mycol=c("darkcyan","indianred")
pdf(file="./severe.results/Figure 6G.mild and severe SLE DEGs.venn.pdf",width=5,height=5)
venn(jj,col=mycol,zcolor=mycol,box=F)
dev.off()

#重型差异基因GO圈图
rt=severe.diffSig %>% rownames_to_column("id")
...
#保存显著富集的结果
write.table(GO,file="./severe.results/severe.GO.txt",sep="\t",quote=F,row.names = F)
...
#  rbind(GO.class) %>% 
#  distinct()
#write.table(GO.class.out,file="./severe.results/severe.GO.class.csv",sep=",",quote=F,row.names = F)

#整理后重新读取
#GO.class1 <- read.table("./severe.results/severe.GO.class1.csv",sep = ",",header = T)  

#library(ggplot2)
#pdf("./severe.results/Figure 6I.severe.GO.class.pdf",width=8,height=8)
...
#绘制圈图的主体部分
pdf("./severe.results/Figure 6H.severe.GO.circlize.pdf",width=10,height=10)
...
zx.celldeath <- bind_rows(data.frame(ferr=celldeath[which(celldeath$ferrgene %in% rownames(severe.diffSig)),]$ferrgene),
                          data.frame(pyro=celldeath[which(celldeath$pyrogene %in% rownames(severe.diffSig)),]$pyrogene),
                          data.frame(anoikis=celldeath[which(celldeath$anoikisgene %in% rownames(severe.diffSig)),]$anoikisgene),
                          data.frame(copper=celldeath[which(celldeath$coppergene %in% rownames(severe.diffSig)),]$coppergene) )
...
write.table(file="./severe.results/zx.celldeath.txt", zx.celldeath, sep="\t", quote=F, col.names=T, row.names=F)


#save.image("severe.diff.RData")
#########################severe SLE immunoinfiltration######################################
rm(list = ls()) #清除缓存
inputFile="C:/Users/Administrator/Desktop/SLE/procedure/step8. GEO/severe.results/severe.exp.txt"      #表达数据文件
...
treatNum <- as.numeric(table(group$group)[3])
...
write.table(outTab3.out, file="./severe.results/severe.CIBERSORT-Results.txt", sep="\t", quote=F, col.names=F)
...
library(vioplot)      
outTab2=data.frame()
pdf(file="./severe.results/Figure 6I.severe.immune.vioplot.pdf", height=8, width=13)
...
#输出免疫细胞和p值表格文件
write.table(immuneDiff,file="./severe.results/severe.immuneDiff.xls",sep="\t",row.names=F,quote=F)

#绘制柱状图
pdf(file="./severe.results/severe.barplot.pdf", width=14.5, height=8.5)
...
#save.image("severe.immune.RData")
