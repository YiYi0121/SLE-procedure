...
#修改1：设定工作目录
setwd("C:/Users/Administrator/Desktop/SLE/procedure/step3. mild SLE core drug")
...
#修改3：挑选高频中药
top <- nrow(herbfreq[which(herbfreq$freq>=20),])
hmod <- herbfreq[which(herbfreq$freq>=20),]$drug
#可视化
#高频中药频数分布图
pdf("./results/Figure 4A.mild SLE. high Frequency.pdf", height = 8, width = 14)
...
##关联规则可视化
pdf("./results/Figure 4B.mild SLE.strongrules TOP50.pdf", height = 8, width = 14) 
# 设置字体大小为1.5
par(cex = 1.5)
...
c<- rbind(b,colsum) 
cc <- c[,which((c[nrow(c),] >6))] #一个药至少在5%以上方子里出现过
...
library(cowplot)
pdf("./results/Figure 4C.mild SLE.best K-value.pdf", height = 4, width = 12)
...
kmod <- as.data.frame(cutree(hc, k=3)) #根据最佳K值修改K值
...
library(ape)
#最佳K值是3
mypal = c("#556270","#4ECDC4","#FF6B6B")
clus3 = cutree(hc,3) #修改K值
pdf("./results/Figure 4D. mild SLE.Cluster Dendrogram.pdf", height = 10, width = 10) 
...
colnames(cmod) <- "drug"
cmod_qx<-merge(cmod,herbfreq,by = 'drug',all.x=TRUE,all.y=FALSE) %>% arrange(desc(freq))
cmod_qx
write.table(cmod_qx,file="./results/mild SLE.core drug.txt",sep="\t",quote=F,col.names=T,row.names = T)
